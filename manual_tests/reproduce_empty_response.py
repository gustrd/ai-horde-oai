...
Investigated a TUI log entry showing status=retry / empty response (raw_len=0)
from worker "Belarrius Studio | RP Worker". The proxy was retrying 6 times and
then returning a GENERATION_FAILURE message to the client.

The script confirmed this is a Horde worker-side bug, not a proxy bug:
- Step 1 (general pool): all workers in the pool returned empty for this input,
  exhausting all retries and triggering GENERATION_FAILURE.
- Step 2 (worker pinned): the specific worker confirmed raw_len=0 consistently
  on the real 18-message / ~7,300-token conversation.

Conclusion: the worker silently returns empty output for long prompts that
approach or exceed its effective context window. The proxy retry logic is
working correctly. No proxy-side fix is needed.

# How to use
1. Export the failing log entry to _repro_entry.json (run from repo root):
     python -c "
     import json; from pathlib import Path
     log = Path.home() / '.ai-horde-oai' / 'requests.jsonl'
     for line in log.read_text('utf-8').splitlines():
         d = json.loads(line)
         if d.get('status') == 'retry' and '21:22' in d.get('timestamp',''):
             Path('_repro_entry.json').write_text(json.dumps(d), 'utf-8'); break
     "
2. Run from repo root (server must be running):
     uv run python manual_tests/reproduce_empty_response.py
